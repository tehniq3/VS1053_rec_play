/***************************************************
  ...
  v.0.0 - initial 
  v.1.0 - niq_ro (Nicu FLORICA) changed some parameters to improve level of noises (also moved ground of output as at https://github.com/karawin/Ka-Radio32/issues/167 )
  v.2.0 - added play and stop buttons
 ****************************************************/
...
void loop() {
  ...
  //  musicPlayer.startRecordWav(false, sampleRate); // use microphone (for linein, pass in 'false')
...
//  if (isRecording && digitalRead(REC_BUTTON)) 
    if (isRecording && !digitalRead(STOP_BUTTON))
    {
